import { eq, and, desc, ilike, sql, inArray, getTableColumns } from "drizzle-orm";
...
export class PostgresColorRepository implements IColorRepository {
  ...
  async list(filter: ColorFilter, ctx: TenantContext): Promise<PaginatedResult<ColorData>> {
    ...
    const keyset: KeysetSpec = { createdAt: colors.createdAt, id: colors.id };
    ...
    const [dataRows, countRows] = await Promise.all([
      this.db
        .select({ ...getTableColumns(colors), ...cursorColumns(keyset) })
        .from(colors)
        .where(pageWhere)
        .limit(limit)
