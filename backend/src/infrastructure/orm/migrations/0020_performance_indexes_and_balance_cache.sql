...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE p.polname = 'tenant_isolation_party_balances' AND c.relname = 'party_balances'
  ) THEN
    CREATE POLICY tenant_isolation_party_balances ON party_balances FOR ALL
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy p
    JOIN pg_class c ON c.oid = p.polrelid
    WHERE p.polname = 'tenant_isolation_ledger_archive' AND c.relname = 'ledger_entry_archive'
  ) THEN
    CREATE POLICY tenant_isolation_ledger_archive ON ledger_entry_archive FOR ALL
      USING (tenant_id = current_setting('app.current_tenant_id')::UUID);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy p
    JOIN pg_class c ON c.oid = p.polrelid
    WHERE p.polname = 'tenant_isolation_yearly_summary' AND c.relname = 'yearly_party_summaries'
  ) THEN
    CREATE POLICY tenant_isolation_yearly_summary ON yearly_party_summaries FOR ALL
      USING (tenant_id = current_setting('app.current_tenant_id')::UUID);
  END IF;
END $$;
