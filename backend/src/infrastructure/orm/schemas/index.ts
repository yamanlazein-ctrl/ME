// Barrel export for all Drizzle schemas
export * from "./migration.table.js";
export * from "./tenant.table.js";
export * from "./user.table.js";
export * from "./party.table.js";
export * from "./fabric.table.js";
export * from "./color.table.js";
export * from "./roll.table.js";
export * from "./invoice.table.js";
export * from "./invoice-line.table.js";
export * from "./order.table.js";
export * from "./order-item.table.js";
export * from "./voucher.table.js";
export * from "./ledger-entry.table.js";
export * from "./cashbox.table.js";
export * from "./expense.table.js";
export * from "./return.table.js";
export * from "./return-line.table.js";
export * from "./print-job.table.js";
export * from "./document-sequence.table.js";
export * from "./document-number-block.table.js";
export * from "./audit-log.table.js";
export * from "./notification.table.js";
export * from "./setting.table.js";
export * from "./attachment.table.js";

// Phase 0 — Platform Foundation
export * from "./license.table.js";
export * from "./license-activation.table.js";
export * from "./device-registration.table.js";
export * from "./sync-device.table.js";
export * from "./sync-device-authorized-user.table.js";
export * from "./sync-outbox.table.js";
export * from "./sync-inbox.table.js";
export * from "./sync-resource-claim.table.js";
export * from "./sync-state.table.js";
export * from "./license-audit-event.table.js";
export * from "./secret.table.js";
export * from "./company-profile.table.js";
export * from "./setup-wizard-state.table.js";
export * from "./server-installation.table.js";
export * from "./system-admin.table.js";
export * from "./invitation-code.table.js";
export * from "./stock-movement.table.js";
export * from "./idempotency-key.table.js";
export * from "./ledger-entry-archive.table.js";
export * from "./yearly-party-summary.table.js";

// P0-004 — durable token revocation (works without Redis)
export * from "./revoked-token.table.js";
