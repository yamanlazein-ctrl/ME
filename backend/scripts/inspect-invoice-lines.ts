...
async function main() {
  for (const num of INV_NUMBERS) {
    ...
    console.log(
      `  subtotal=${inv.subtotal}, discount=${inv.discount}, tax=${inv.tax}, shipping=${inv.shipping}`,
    );
    console.log(
      `  total=${inv.total}, paid=${inv.paid}, paymentMethod=${inv.paymentMethod ?? "null"}`,
    );
    ...
    for (const l of lines) {
      ...
      console.log(
        `  Line: fabric=${fab[0]?.name ?? "?"}, color=${col[0]?.name ?? "?"}, roll=${rol[0]?.rollNo ?? "?"}`,
      );
      console.log(
        `        qty=${l.quantityKg}, price=${l.pricePerKg}, pieces=${l.pieces}, discountAmount=${l.discountAmount}`,
      );
