    ...
    >
      ...
      <div className="relative">
        ...
        <div className="mt-4 grid grid-cols-2 gap-x-3 gap-y-3 border-t border-primary/10 pt-3 sm:grid-cols-1 md:grid-cols-3">
          ...
          <HeroChip icon={Layers} label="قطعة نشطة" value={formatMoney(activeRolls?.total ?? 0)} />
          <HeroChip
            icon={PackageOpen}
...
function HeroChip({
  ...
  if (to) {
    // Whole card is clickable → navigates to the given route (same design).
    return (
  ...
  return <div className="flex items-center gap-2 min-w-0">{inner}</div>;
}
