...
import { addColor, addFabric, addRoll, fabricById, updateColor, updateFabric, updateRoll, type Color, type Currency, type Fabric, type Roll } from "@/presentation/hooks/useInventory";
...
function FabricFormDialog({ state, onClose }: { state: FabricFormState; onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    setCatErr(null);
    let valid = true;
    if (!name.trim()) { setNameErr("اسم القماش مطلوب."); valid = false; }
    if (!category.trim()) { setCatErr("الفئة مطلوبة."); valid = false; }
    if (!valid) return;
    try {
      if (editing) {
          ...
          <SectionCard index={1} title="المعلومات الأساسية" desc="تعريف القماش وفئته ووحدة القياس.">
            ...
            <Field label="الفئة" required error={catErr ?? undefined}>
              <Input
                className="h-12"
                value={category}
                onChange={(e) => { setCategory(e.target.value); setCatErr(null); }}
...
function ColorFormDialog({ state, onClose }: { state: ColorFormState; onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    setCodeErr(null);
    let valid = true;
    if (!name.trim()) { setNameErr("اسم اللون مطلوب."); valid = false; }
    if (!code.trim()) { setCodeErr("رقم اللون مطلوب."); valid = false; }
    if (!valid) return;
    try {
      ...
      <DialogContent dir="rtl" className="max-w-md">
        ...
        <div className="grid gap-3">
          <div className="flex items-center gap-3">
            <ColorSwatch color={{ name, code, hex: hex ?? null, imageUrl: imageUrl ?? null }} size="lg" />
            <div className="flex-1">
...
function RollFormDialog({ state, onClose }: { state: RollFormState; onClose: () => void }) {
  ...
  const submit = async () => {
    setRollErr(null); setDyeErr(null); setQtyErr(null);
    let valid = true;
    if (!rollNo.trim()) { setRollErr("رقم الصبغة مطلوب."); valid = false; }
    if (!dyeBatch.trim()) { setDyeErr("رقم الدفعة الصبغية مطلوب."); valid = false; }
    if (!qty || qty <= 0) { setQtyErr("أدخل كمية صحيحة أكبر من صفر."); valid = false; }
    if (!valid) return;
    try {
      if (editing) {
      ...
      <DialogContent dir="rtl" className="max-w-xl">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Input value={rollNo} onChange={(e) => { setRollNo(e.target.value); setRollErr(null); }} />
            {rollErr && <p className="mt-1 text-[11px] text-destructive">{rollErr}</p>}
          </div>
          <div>
            <Label>رقم الصبغة *</Label>
            <Input value={dyeBatch} onChange={(e) => { setDyeBatch(e.target.value); setDyeErr(null); }} />
