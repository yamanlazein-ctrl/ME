  ...
  const save = async () => {
    ...
    try {
      const o = await createOrder.mutateAsync({
        ...
        customerPhoneSnapshot: cust?.phone ?? undefined,
        date,
        notes,
        currency,
        items: valid.map((l) => {
          const fabricName = l.fabricName.trim();
          const colorName = l.colorName.trim();
          // Auto-link the typed fabric/color to an existing inventory color so
          // the order carries a real `colorId` and can be matched to entry
          // invoices later (notifyOrderAvailability). If nothing matches (e.g.
          // a brand-new color name), it stays name-only — same as before.
          let fabricId = l.fabricId || undefined;
      ...
      setError(det || (e as Error).message);
    }
  };
