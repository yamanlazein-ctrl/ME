...
## ADR-DESKTOP-2026 — غلاف سطح المكتب: Tauri أم Electron، وبنية التشغيل

**الحالة:** قرار معتمد (2026-09-21). يحلّ محل تصميم «ثلاث عمليات + منافذ ثابتة + بصمة عتاد متعددة الإشارات».

**السياق (أعطال مؤكَّدة عند الزبون):** تثبيت يتجاوز 10 دقائق، إقلاع معلَّق «جاري تشغيل النظام…» أكثر من دقيقتين،
منفذ 8080 لا يعمل، وخطأ «تغيّر تعريف الجهاز». الأسباب الجذرية (مقاسة/مقروءة من الكود، لا انطباعية):

| العَرَض | السبب الجذري |
|---|---|
| تثبيت > 10 دقائق | حزمة من **33,598 ملفاً / 494 MB** (19,179 ملف `node_modules` للباك-إند + 9,863 لـSSR) داخل **MSI** الذي يسجّل كل ملف ويحفظ نسخة تراجع له، مع فحص Defender الفوري لكل ملف |
| إقلاع معلَّق | ثلاث عمليات متتالية (Postgres ← Node/backend بمهلة 60 ث ← Node/SSR بمهلة 120 ث)؛ كل ملف يُفحص عند أول لمس؛ أي بطء ينتهي بخطأ قاتل ثم قتل كل شيء |
| المنفذ | SSR على **4173 ثابت بلا بديل** وعنوان النافذة مكتوب حرفياً، والباك-إند مفضّلاً 8080؛ أي برنامج آخر (أو نسخة يتيمة من التطبيق) على المنفذ = انتظار ثم فشل |
| «تغيّر تعريف الجهاز» | البصمة = هاش (اسم الجهاز + أول MAC من `getmac` + اسم المعالج من `wmic` + MachineGuid)؛ تغيّر أيّ منها (إعادة تسمية، VPN، غياب `wmic` في Windows 11 الحديث) يوقف التشغيل نهائياً |

**القرار 1 — الإبقاء على Tauri 2 (لا Electron).** لا شيء من الأعطال الأربعة سببه إطار الغلاف؛ Electron كان سيحمل نفس
الأسباب (Node + PostgreSQL + آلاف الملفات + منافذ). مقارنة موجزة:

| | Tauri 2 (المعتمد) | Electron |
|---|---|---|
| حجم الغلاف | ~10 MB، يستخدم WebView2 المثبّت مع Windows | ~250 MB (Chromium كاملاً) + ~150 MB RAM إضافية |
| الباك-إند | عملية Node واحدة (نفسها التي تخدم الويب) | يمكن تشغيله داخل العملية الرئيسية، لكن هذا يوفّر عملية واحدة فقط ويربط دورة حياة الخادم بالنافذة |
| كلفة الانتقال | لا شيء | إعادة كتابة ~3.9k سطر Rust مُختبَرة (DPAPI للأسرار، إدارة العمليات، تجهيز القاعدة، تعافٍ) ثم إعادة التحقق الكاملة |
| التحديث | `tauri-plugin-updater` (جاهز، يحتاج مفتاح توقيع) | `electron-updater` (ناضج) |
| المخاطرة | WebView2 قد يكون غائباً على Windows 10 قديم (يُعالَج بـ`embedBootstrapper`) | مسؤولية ترقيع Chromium على عاتقنا |

يُعاد النظر فقط إذا ثبت من الميدان أن WebView2 مشكلة متكررة عند الزبائن.

**القرار 2 — بنية تشغيل جديدة:** عمليتان فقط: **خادم Node واحد** (يخدم الـAPI **والواجهة الثابتة** على نفس الأصل والمنفذ)
+ **PostgreSQL**. لا SSR (التطبيق بلا server functions)، لا CORS، لا وكيل.
- منفذ الخادم عشوائي عالٍ يُختار **مرة واحدة** ويُحفَظ ويُعاد استخدامه (أصل المتصفح ثابت فتبقى الجلسة وإعدادات الجهاز بعد إعادة
  التشغيل)، ويتغيّر فقط إذا كان مشغولاً فعلاً؛ لا 8080/4173. الخادم يكتب منفذه في `server-port.json` **بعد** أن يصبح جاهزاً فعلاً؛
  والنافذة تُنشأ وقتها.
- الباك-إند يُجمَّع بـesbuild في ملف واحد (~100 ملف مع argon2 الأصلي والهجرات بدل 19 ألفاً).
- PostgreSQL مُقلَّم (بلا ترجمات/وثائق/نسخة مكررة): 4,274 ← 2,760 ملفاً. الإجمالي ~3 آلاف ملف بدل 33.6 ألفاً.
- المثبّت **NSIS لكل مستخدم** (بلا UAC ولا MSI).
- الانتظار قائم على **حياة العملية** لا على مهل: جاهز ← تابع؛ مات ← فشل فوراً برمز الخروج وآخر سطور السجل؛ حيّ وبطيء ← استمر
  مع عرض الوقت المنقضي (سقف أمان 20 دقيقة لعملية معلّقة فعلاً فقط).
- نسخة واحدة لكل مستخدم (single-instance) حتى لا تتنازع نسختان على مجلد قاعدة البيانات.

**القرار 3 — ربط الجهاز:** إشارة واحدة ثابتة (`MachineGuid`) عبر واجهة السجل (بلا `getmac`/`wmic`). تغيّرها **لا يوقف** التطبيق؛
يُحدَّث الربط تلقائياً. الحماية من النسخ هي تشفير DPAPI **لكل مستخدم** لملف الربط والأسرار (الترخيص أصلاً غير مربوط بجهاز).

**القرار 4 — تعدد الأجهزة والويب:** لكل جهاز سطح مكتب قاعدته المحلية وتزامنها مع خادم مركزي (آلية المزامنة القائمة)؛ والأجهزة
الإضافية تستخدم **نسخة الويب نفسها** كتطبيق Windows عبر PWA (Edge/Chrome ← «تثبيت التطبيق»): `manifest.webmanifest` + `sw.js` محافظ
(لا يخزّن أبداً `/api/*`).

**لماذا لا نُرقِّع:** الأسباب بنيوية (حجم الحزمة، طوبولوجيا العمليات، منافذ ثابتة، بصمة هشّة) — أيّ ترقيع فوقها يعيد نفس
الأعراض بأشكال أخرى.
