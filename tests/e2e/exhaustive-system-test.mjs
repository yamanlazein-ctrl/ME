...
function check(tag, name, expected, actual, detail = "") {
  ...
  console.log(
    `[${pass ? "✓" : "✗"}] [${tag}] ${name}${!pass ? `  (expected=${JSON.stringify(expected)} actual=${JSON.stringify(actual)})` : ""}${detail ? "  " + detail : ""}`,
  );
}

async function login() {
  token = (
    await api("POST", "/api/auth/login", { email: EMAIL, password: PASSWORD, tenantId: TENANT_ID })
  ).data.accessToken;
}
async function ledgerByRef(t, id) {
  return (
    (await api("GET", `/api/ledger?referenceType=${t}&referenceId=${id}&limit=1000`)).data?.data ??
    []
  );
}
async function rollKg(id) {
  const r = await api("GET", `/api/inventory/rolls/${id}`);
  return Number(r.data?.remainingKg ?? 0);
}
async function cashbox() {
  const r = await api("GET", `/api/cashbox/balance/${today()}?currency=SYP`);
  return typeof r.data === "number" ? r.data : (r.data?.balance ?? 0);
}

async function mkStock(name, cost, kg) {
  const fab = (await api("POST", "/api/inventory/fabrics", { name, minStockKg: 5 })).data;
  const col = (
    await api("POST", "/api/inventory/colors", {
      fabricId: fab.id,
      name: name + "-color",
      code: `C${uniq()}`,
    })
...
async function stage1() {
  ...
  // INV.BC.01: sell exactly available
  {
    ...
    const r = await api("POST", "/api/invoices", {
      ...
      currency: "SYP",
      lines: [
        { fabricId: fab.id, colorId: col.id, rollId: roll.id, quantityKg: 50, pricePerKg: 1000 },
      ],
    });
    check(
      "I10",
      "INV.BC.01 sell exactly available → 0 remaining",
      true,
      r.status === 201 && (await rollKg(roll.id)) === 0,
    );
  }
  // INV.BC.02: sell available + 0.01 (scale-2 granularity)
  {
    ...
    const r = await api("POST", "/api/invoices", {
      ...
      currency: "SYP",
      lines: [
        { fabricId: fab.id, colorId: col.id, rollId: roll.id, quantityKg: 50.01, pricePerKg: 1000 },
      ],
    });
  ...
  // INV.BC.03: sell 0
  {
    ...
    const r = await api("POST", "/api/invoices", {
      ...
      currency: "SYP",
      lines: [
        { fabricId: fab.id, colorId: col.id, rollId: roll.id, quantityKg: 0, pricePerKg: 1000 },
      ],
    });
  ...
  // INV.BC.04: sell negative
  {
    ...
    const r = await api("POST", "/api/invoices", {
      ...
      currency: "SYP",
      lines: [
        { fabricId: fab.id, colorId: col.id, rollId: roll.id, quantityKg: -5, pricePerKg: 1000 },
      ],
    });
  ...
  {
    const fab = (
      await api("POST", "/api/inventory/fabrics", { name: `قماش ${u}-5`, minStockKg: 5 })
    ).data;
    const col = (
      await api("POST", "/api/inventory/colors", {
        fabricId: fab.id,
  ...
  // INV.BC.06: 0.001 kg line (beyond 2dp storage precision) → rejected
  {
    ...
    const r = await api("POST", "/api/invoices", {
      ...
      currency: "SYP",
      lines: [
        { fabricId: fab.id, colorId: col.id, rollId: roll.id, quantityKg: 0.001, pricePerKg: 1000 },
      ],
    });
    check(
      "I4",
      "INV.BC.06 0.001 kg line → rejected (precision 0.01)",
      true,
      r.status === 422,
      `status=${r.status}`,
    );
  ...
  {
    const r = await api("POST", "/api/cashbox/opening-balance", {
      openingBalance: 0,
      openingDate: today(),
      currency: "SYP",
    });
  ...
  {
    const funder = (await api("POST", "/api/customers", { name: `ممول ${u}`, code: `F-${uniq()}` }))
      .data;
    ...
    check(
      "CB",
      "CB.BC.02 expense > cashbox balance → rejected (no negative)",
      true,
      r.status === 422 && after === before,
  ...
  {
    const funder = (
      await api("POST", "/api/customers", { name: `تمويل ${u}`, code: `T-${uniq()}` })
    ).data;
    await api("POST", "/api/receipts", {
      kind: "receipt",
      date: today(),
    ...
    check(
      "CB",
      "CB.GEN.01 cashbox funded (10,000,500) for later stages",
      true,
      b === 10000500,
      `balance=${b}`,
    );
  ...
  {
    const a = (await api("POST", "/api/customers", { name: `عميل A ${u}`, code: `A-${uniq()}` }))
      .data;
    const b = (await api("POST", "/api/suppliers", { name: `مورد B ${u}`, code: `B-${uniq()}` }))
      .data;
    const before = await cashbox();
    const rec = await api("POST", "/api/receipts", {
      kind: "receipt",
      date: today(),
      partyId: a.id,
      partyKind: "customer",
    ...
    check(
      "CB",
      "CB.ED.01 receipt+payment same moment → both balanced, cashbox net +2000",
      true,
      rec.status === 201 &&
        pay.status === 201 &&
        dR === cR &&
  ...
  // CB.ED.02: partial payment in USD on a USD invoice — I7 cross-check
  {
    ...
    await api("POST", "/api/invoices", {
      ...
      currency: "USD",
      lines: [
        { fabricId: fab.id, colorId: col.id, rollId: roll.id, quantityKg: 10, pricePerKg: 5 },
      ],
    });
    await api("POST", "/api/receipts", {
      kind: "receipt",
    ...
    check(
      "CB",
      "CB.ED.02 partial USD payment on USD invoice → USD=30, SYP=0 (I7)",
      true,
      usd.balance === 30 && syp.balance === 0,
      `USD=${usd.balance} SYP=${syp.balance}`,
    );
  }
  // RT.BC.01: return full invoice quantity
  {
    ...
    const inv = (
      await api("POST", "/api/invoices", {
        ...
        currency: "SYP",
        lines: [
          { fabricId: fab.id, colorId: col.id, rollId: roll.id, quantityKg: 20, pricePerKg: 1000 },
        ],
      })
    ).data;
    const r = await api("POST", "/api/returns", {
      ...
      reason: "defect",
      currency: "SYP",
      lines: [{ rollId: roll.id, quantityKg: 20, pricePerKg: 1000 }],
    });
  ...
  // RT.BC.02: return > sold → rejected
  {
    ...
    const inv = (
      await api("POST", "/api/invoices", {
        ...
        currency: "SYP",
        lines: [
          { fabricId: fab.id, colorId: col.id, rollId: roll.id, quantityKg: 10, pricePerKg: 1000 },
        ],
      })
    ).data;
    const r = await api("POST", "/api/returns", {
      ...
      reason: "defect",
      currency: "SYP",
      lines: [{ rollId: roll.id, quantityKg: 15, pricePerKg: 1000 }],
    });
  ...
  // INV.ED.01: print send ALL available stock → remaining 0 (no negative)
  {
    ...
    const s = await api("POST", "/api/printing/send", {
      date: today(),
    ...
    check(
      "I9",
      "INV.ED.01 print send ALL available stock → remaining 0",
      true,
      s.status === 201 && remaining === 0,
      `status=${s.status} remaining=${remaining}`,
    );
  }
  // INV.ED.02: cancel sale then partial return on it → rejected; stock restored
  {
    ...
    const inv = (
      await api("POST", "/api/invoices", {
        ...
        currency: "SYP",
        lines: [
          { fabricId: fab.id, colorId: col.id, rollId: roll.id, quantityKg: 30, pricePerKg: 1000 },
        ],
      })
    ).data;
    const c = await api("POST", `/api/invoices/${inv.id}/cancel`);
    const restored = await rollKg(roll.id);
    const rr = await api("POST", "/api/returns", {
      kind: "sale",
      date: today(),
      partyId: cust.id,
      originalInvoiceId: inv.id,
      reason: "defect",
      currency: "SYP",
      lines: [{ rollId: roll.id, quantityKg: 10, pricePerKg: 1000 }],
    });
    check(
      "RT",
      "INV.ED.02 cancel sale then partial return → rejected, stock restored",
      true,
      c.status === 200 && restored === 100 && rr.status === 422,
      `cancel=${c.status} restored=${restored} return=${rr.status}`,
    );
  }
  // RT.ED.01: two returns on same invoice exceeding original qty → second rejected
  {
    ...
    const r1 = await api("POST", "/api/returns", {
      kind: "sale",
      date: today(),
      partyId: cust.id,
      originalInvoiceId: inv.id,
    ...
    const r2 = await api("POST", "/api/returns", {
      kind: "sale",
      date: today(),
      partyId: cust.id,
      originalInvoiceId: inv.id,
    ...
    check(
      "RT",
      "RT.ED.01 two returns cumulative > original → second rejected",
      true,
      r1.status === 201 && r2.status === 422,
  ...
  // PR.ED.01: print send then exact receive → conservation, waste 0
  {
    ...
    const recv = await api("POST", "/api/printing/receive", {
      jobId: send.id,
      date: today(),
      receivedKg: 40,
      newName: "مطبوع",
    ...
    check(
      "I9",
      "PR.ED.01 print send then exact receive → source 60, waste 0",
      true,
      srcAfter === 60 && [200, 201].includes(recv.status) && job.wasteKg === 0,
      `src=${srcAfter} status=${recv.status} waste=${job.wasteKg}`,
    );
  }
  // PR.BC.01: print over-receive rejected
  {
    ...
    const r = await api("POST", "/api/printing/receive", {
      jobId: send.id,
      date: today(),
      receivedKg: 31,
    });
...
async function stage2() {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      if (kind === 0) {
        ...
        const r = await api("POST", "/api/invoices", {
          ...
          currency: "SYP",
          lines: [
            { fabricId: fab.id, colorId: col.id, rollId: roll.id, quantityKg: 2, pricePerKg: 800 },
          ],
        });
  ...
  // I2: quantity conservation across 30 random ops (sales + returns)
  {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if ((await rollKg(roll.id)) !== expected) {
        conserved = false;
        break;
      }
      const rr = await api("POST", "/api/returns", {
        ...
        reason: "defect",
        currency: "SYP",
        lines: [{ rollId: roll.id, quantityKg: qty, pricePerKg: 1000 }],
      });
      if (rr.status !== 201) {
        conserved = false;
        break;
    ...
    check(
      "I2",
      "I2.GEN.01 quantity conservation across 30 random ops",
      true,
      conserved,
      `remaining=${await rollKg(roll.id)} expected=${expected}`,
    );
  }
  ...
  // I2.GEN.02: stock-in (purchase/entry invoice) conservation — every kg added is tracked
  {
    ...
    for (const qty of [25.5, 10, 4.25, 0.75]) {
      const r = await api("POST", "/api/invoices", {
        ...
        currency: "SYP",
        lines: [
          { fabricId: fab.id, colorId: col.id, rollId: roll.id, quantityKg: qty, pricePerKg: 800 },
        ],
      });
      if (r.status !== 201) {
        conserved = false;
    ...
    check(
      "I2",
      "I2.GEN.02 stock-in (purchase) conservation across 4 entry invoices",
      true,
      conserved,
      `remaining=${await rollKg(roll.id)} expected=${expected}`,
    );
  ...
  // I2.GEN.03: fractional 2dp quantities conservation (10 sale/return pairs)
  {
    ...
    check(
      "I2",
      "I2.GEN.03 fractional 2dp conservation across 10 sale/return pairs",
      true,
      conserved,
      `remaining=${await rollKg(roll.id)} expected=${expected}`,
    );
  ...
  {
    const fab = (
      await api("POST", "/api/inventory/fabrics", { name: `قماش ${u}-I8`, minStockKg: 5 })
    ).data;
    const cols = [],
      rolls = [];
    for (let i = 0; i < 6; i++) {
  ...
  // I10: 20 random over-sell attempts all rejected, no negative
  {
    ...
    check(
      "I10",
      "I10.GEN.01 20 over-sell attempts all rejected, stock unchanged",
      true,
      rejected === 20 && kg === 30,
      `rejected=${rejected} kg=${kg}`,
    );
  }
  ...
  // I7: currency isolation
  {
    ...
    check(
      "I7",
      "I7.GEN.01 USD balance 50, SYP balance 0 (no mixing)",
      true,
      usdBal.balance === 50 && sypBal.balance === 0,
      `USD=${usdBal.balance} SYP=${sypBal.balance}`,
    );
  }
  ...
  // I6: dashboard profit == ledger revenue − COGS
  {
    ...
    const rev = all
      .filter((e) => e.type === "sales_revenue" && e.status === "active")
      .reduce((s, e) => s + (e.credit || 0), 0);
    const cogs = all
      .filter((e) => e.type === "cogs_expense" && e.status === "active")
      .reduce((s, e) => s + (e.debit || 0), 0);
    const dash = (await api("GET", "/api/dashboard")).data;
    check(
      "I6",
      "I6.GEN.01 dashboard profit == ledger revenue − COGS",
      rev - cogs,
      dash.todayProfit?.syp,
    );
  }

  // I4: 300 sequential money ops → no drift, exact integer balances
  {
    const c4 = (await api("POST", "/api/customers", { name: `عميل I4 ${u}`, code: `I4-${u}` }))
      .data;
    let total = 0;
    let allOk = true;
    for (let i = 0; i < 300; i++) {
    ...
    const all =
      (await api("GET", `/api/ledger?partyId=${c4.id}&sort=asc&limit=1000`)).data?.data ?? [];
    const replay = all
    ...
    check(
      "I4",
      "I4.GEN.01 300 sequential ops → no drift, exact integer balance",
      true,
      allOk && exact && consistent,
      `total=${total} replay=${replay} api=${bal.balance}`,
    );
  }
  ...
  {
    const all =
      (await api("GET", `/api/ledger?partyId=${cust.id}&sort=asc&limit=1000`)).data?.data ?? [];
    const replay = all
      .filter((e) => e.status === "active")
      .reduce((s, e) => s + (e.debit || 0) - (e.credit || 0), 0);
    ...
    check(
      "I5",
      "I5.GEN.01 replay balance == API balance",
      apiBal.balance,
      replay,
      `api=${apiBal.balance} replay=${replay}`,
    );
  }
  ...
  // I3: idempotency
  {
    ...
    check(
      "I3",
      "I3.GEN.01 duplicate invoice (same key) → 1 invoice",
      1,
      count,
  ...
  {
    const c3 = (await api("POST", "/api/customers", { name: `عميل IE3 ${u}`, code: `IE3-${u}` }))
      .data;
    ...
    const all =
      (await api("GET", `/api/ledger?partyId=${c3.id}&sort=asc&limit=1000`)).data?.data ?? [];
    const allCancelled = all.length > 0 && all.every((e) => e.status === "cancelled");
    check(
      "RT",
      "INV.ED.03 cancel invoice after partial receipt → stock restored, balance 0, legs cancelled",
      true,
...
async function stage3() {
  ...
  check(
    "CC",
    "CC.01 10 concurrent 10kg sales on 50kg roll → 5 succeed, remaining 0",
    true,
    succeeded === 5 && kg === 0,
    `succeeded=${succeeded} kg=${kg} statuses=${outs.map((o) => o.status).join(",")}`,
  );
  ...
  check(
    "CC",
    "CC.02 concurrent duplicate receipt (same key) → 1 voucher",
    true,
    (a.status === 201 || b.status === 201) && recv.length === 1,
    `a=${a.status} b=${b.status} count=${recv.length}`,
  );

  // CC.03: 20 concurrent receipts on same customer → no lost updates, balance exact
  {
    const c3 = (await api("POST", "/api/customers", { name: `عميل CC3 ${u}`, code: `CC3-${u}` }))
      .data;
    ...
    check(
      "CC",
      "CC.03 20 concurrent receipts same customer → no lost updates",
      true,
      okCount === 20 && bal.balance === replay && Math.abs(replay) === 20000,
      `ok=${okCount} replay=${replay} api=${bal.balance}`,
    );
  }
  ...
  // LD.01: 300 sequential ops latency
  {
    ...
    check(
      "LD",
      "LD.01 300 ops complete, latency stable",
      true,
      ok === 300,
      `${ms}ms total (${(ms / 300).toFixed(1)}ms/op)`,
    );
  }
...
async function stage4() {
  ...
  for (const p of parties) {
    const all =
      (await api("GET", `/api/ledger?partyId=${p.id}&sort=asc&limit=1000`)).data?.data ?? [];
  ...
  check(
    "RP",
    "RP.01 replay ALL party balances == API",
    true,
    allMatch,
    `parties=${parties.length} currency-checks=${checks}`,
  );
...
async function report() {
  ...
  await writeFile(
    path.join(AUDIT_DIR, "exhaustive-report.json"),
    JSON.stringify({ results, counters }, null, 2),
  );
  ...
  if (fails.length === 0) lines.push("- none");
    ...
    for (const f of fails)
      lines.push(`- [${f.tag}] ${f.name}: expected \`${f.expected}\` got \`${f.actual}\``);
  lines.push("");
  const totalFail = Object.values(counters).reduce((s, c) => s + c.fail, 0);
  lines.push(
    `## Verdict: ${totalFail === 0 ? "ALL INVARIANTS HOLD under tested conditions" : "INVARIANTS BROKEN — see failures above"}`,
  );
  lines.push("");
  ...
  lines.push(
    "- I6 (off-by-1 COGS): journal rounds quantity to the DB's 2dp scale; dashboard COGS uses per-line ROUND() so profit is an exact integer matching the ledger.",
  );
  lines.push(
    "- I3/CC.02 (idempotency race): key is now claimed atomically (Redis SET NX / in-process check-and-set) before the handler runs; a truly-concurrent duplicate gets HTTP 409 DUPLICATE_IN_FLIGHT instead of double-processing.",
  );
  lines.push(
    "- CB.BC.02 (negative cashbox): expenses that would overdraw a matching-currency cashbox session are now rejected (422).",
  );
  lines.push(
    "- I4 (sub-granularity): invoice/return/print/roll schemas reject quantities & prices with more than 2 decimal places (matches decimal(12,2) storage).",
  );
