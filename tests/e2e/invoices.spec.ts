...
test.describe("فاتورة دخول", () => {
  ...
  test("create entry invoice → 201", async () => {
    const { status, data } = await api("/invoices", {
      ...
      body: JSON.stringify({
        ...
        partyType: "supplier",
        currency: "SYP",
        lines: [{ fabricId: fid, colorId: cid, rollId: rid, quantityKg: 5, pricePerKg: 2000 }],
      }),
    });
  ...
  test("reject negative qty → 422", async () => {
    const { status } = await api("/invoices", {
      method: "POST",
      body: JSON.stringify({
        type: "entry",
        date: "2026-08-12",
        ...
        partyType: "supplier",
        currency: "SYP",
        lines: [{ fabricId: fid, colorId: cid, rollId: rid, quantityKg: -5, pricePerKg: 100 }],
      }),
    });
      ...
      test("create sale invoice → 201 + stock deducted", async () => {
        const { status, data } = await api("/invoices", {
          ...
          body: JSON.stringify({
            ...
            partyType: "customer",
            currency: "SYP",
            lines: [{ fabricId: fid, colorId: col, rollId: rid, quantityKg: 3, pricePerKg: 5000 }],
          }),
        });
        expect(status).toBe(201);
      ...
      test("reject insufficient stock → 422", async () => {
        const { status } = await api("/invoices", {
          ...
          body: JSON.stringify({
            ...
            partyType: "customer",
            currency: "SYP",
            lines: [{ fabricId: fid, colorId: col, rollId: rid, quantityKg: 99999, pricePerKg: 1 }],
          }),
        });
        expect(status).toBe(422);
      });
