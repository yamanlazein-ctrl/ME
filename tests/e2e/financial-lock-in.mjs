 ...
 *
 * Base URL override:  ERP_API_BASE_URL=http://127.0.0.1:8080
 *                     (default must match backend PORT — backend/.env → 8080)
 *
...
async function ledgerByRef(refType, refId) {
  const r = await api(
    "GET",
    `/api/ledger?referenceType=${refType}&referenceId=${refId}&limit=1000`,
  );
  return r.data?.data ?? [];
}
...
async function mkStock(cost = 5000, kg = 500) {
  const u = uniq();
  const fab = (await api("POST", "/api/inventory/fabrics", { name: `قماش ${u}`, minStockKg: 10 }))
    .data;
  const col = (
...
await test("Problem 1a — every transaction is double-entry balanced", async () => {
  await login();
  const u = uniq();
  const customer = (await api("POST", "/api/customers", { name: `عميل ${u}`, code: `CUST-${u}` }))
    .data;
  const supplier = (await api("POST", "/api/suppliers", { name: `مورد ${u}`, code: `SUPP-${u}` }))
    .data;
  ...
  for (const [refType, refId] of cases) {
    ...
    ok(
      `${refType} balanced (Σdebit=Σcredit, ≥2 legs)`,
      balanced,
      `entries=${entries.length} D=${debit} C=${credit}`,
    );
...
await test("Problem 1b — COGS is journaled per color (not averaged)", async () => {
  ...
  const customer = (await api("POST", "/api/customers", { name: `عميل ${u}`, code: `CUST-${u}` }))
    .data;
  const fab = (await api("POST", "/api/inventory/fabrics", { name: `قماش ${u}`, minStockKg: 5 }))
    .data;
  const colA = (
    await api("POST", "/api/inventory/colors", {
      fabricId: fab.id,
  ...
  const saleA = (
    await api("POST", "/api/invoices", {
      ...
      currency: "SYP",
      lines: [
        { fabricId: fab.id, colorId: colA.id, rollId: rollA.id, quantityKg: 30, pricePerKg: 7000 },
      ],
    })
  ).data;
  const saleB = (
    await api("POST", "/api/invoices", {
      ...
      currency: "SYP",
      lines: [
        { fabricId: fab.id, colorId: colB.id, rollId: rollB.id, quantityKg: 20, pricePerKg: 7000 },
      ],
    })
  ).data;
  ...
  const cogsDebitA = legsA
    .filter((e) => e.type === "cogs_expense")
    .reduce((s, e) => s + (e.debit || 0), 0);
  const cogsDebitB = legsB
    .filter((e) => e.type === "cogs_expense")
    .reduce((s, e) => s + (e.debit || 0), 0);
  const invCreditA = legsA
    .filter((e) => e.type === "inventory_asset")
  ...
  ok(
    "Two colors → two distinct COGS entries (not averaged)",
    cogsA !== cogsB && cogsDebitA === cogsA && cogsDebitB === cogsB,
  );
  ...
  const cogs = all
    ...
    .reduce((s, e) => s + (e.debit || 0), 0);
  ok(
    "Profit two ways match (dashboard == ledger revenue−COGS)",
    profitDash === rev - cogs,
    `dash=${profitDash} ledger=${rev - cogs}`,
  );
});
...
await test("Problem 2 — invoice date filter + ordering + inclusive bounds", async () => {
  ...
  for (const off of [-2, 0, -4, -1, -3]) {
    ...
    await api("POST", "/api/invoices", {
      ...
      currency: "SYP",
      lines: [
        { fabricId: fab.id, colorId: col.id, rollId: roll.id, quantityKg: 10, pricePerKg: 100 },
      ],
    });
  }
  const range = (
    await api(
      "GET",
      `/api/invoices?partyId=${customer.id}&fromDate=${dOff(-3)}&toDate=${dOff(-1)}&limit=1000`,
    )
  ).data.data;
  const dates = range.map((i) => i.date);
  ok("Range returns exactly 3 invoices", range.length === 3, `got ${range.length}`);
  ok(
    "Range excludes out-of-range invoices",
    dates.every((d) => d >= dOff(-3) && d <= dOff(-1)),
  );
  ok(
    "Ordering is chronological by date",
  ...
  const single = (
    await api(
      "GET",
      `/api/invoices?partyId=${customer.id}&fromDate=${dOff(-2)}&toDate=${dOff(-2)}&limit=1000`,
    )
  ).data.data;
  ok(
    "Inclusive boundary (from=to=-2) returns 1",
    single.length === 1 && single[0].date === dOff(-2),
    `got ${single.length}`,
  );
});
...
await test("Problem 3 — transfer expenses reduce the cashbox", async () => {
  ...
  await api("POST", "/api/cashbox/opening-balance", {
  ...
  for (const e of expenses)
    await api("POST", "/api/expenses", { ...e, currency: "SYP", date: today() });
  ...
  ok(
    "Cashbox reduced by all transfer expenses",
    balance === opening - total,
    `balance=${balance} expected=${opening - total}`,
  );
  ok(
    "No 1,455,000 residual",
    balance !== opening,
    `balance=${balance} (must not stay at ${opening})`,
  );
});
