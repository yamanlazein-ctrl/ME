...
await test("فاتورة دخول", async () => {
  const { status, data } = await api("/invoices", {
    ...
    body: JSON.stringify({
      ...
      partyType: "supplier",
      currency: "SYP",
      lines: [{ fabricId: FID, colorId: CCOL, rollId: RID, quantityKg: 5, pricePerKg: 2000 }],
    }),
  });
...
await test("فاتورة بيع", async () => {
  ...
  const { status, data } = await api("/invoices", {
    ...
    body: JSON.stringify({
      ...
      partyType: "customer",
      currency: "SYP",
      lines: [{ fabricId: FID, colorId: CCOL, rollId: RID, quantityKg: 3, pricePerKg: 5000 }],
    }),
  });
...
await test("سجل المرتجعات", async () => {
  ...
  ok(
    "Filter entry",
    (await api("/returns?kind=entry&limit=3")).data?.data?.every((r) => r.kind === "entry"),
  );
  ok(
    "Filter sale",
    (await api("/returns?kind=sale&limit=3")).data?.data?.every((r) => r.kind === "sale"),
  );
...
await test("سندات", async () => {
  ...
  ok(
    "Cancel receipt → 200",
    (await api(`/receipts/${data.id}/cancel`, { method: "POST" })).status === 200,
  );
  ...
  ok(
    "Cancel payment → 200",
    (await api(`/payments/${p.data.id}/cancel`, { method: "POST" })).status === 200,
  );
});
...
await test("دفتر حركات + صندوق", async () => {
  ...
  ok(
    "Cashbox movements → 200",
    (await api("/cashbox/movements/2026-08-12?currency=SYP")).status === 200,
  );
});
...
await test("إرسال واستلام مطبعة", async () => {
  ...
  ok(
    "In list",
    (await api("/printing")).data?.some((j) => j.id === data.id),
  );
  ok(
    "In open",
    (await api("/printing/open")).data?.some((j) => j.id === data.id),
  );
  const { status: s2, data: rec } = await api("/printing/receive", {
    method: "POST",
    body: JSON.stringify({ jobId: data.id, date: "2026-08-12", receivedKg: 3, currency: "SYP" }),
  });
  ...
  ok(
    "Result roll created",
    r2.data.some((r) => r.rollNo?.startsWith("PRT-") && r.remainingKg === 3),
  );
});
...
await test("طلبات العملاء", async () => {
  const { status, data } = await api("/orders", {
    method: "POST",
    body: JSON.stringify({
      customerNameSnapshot: "اختبار",
      date: "2026-08-12",
      currency: "SYP",
      items: [{ fabricName: "جاكار", colorName: "أحمر", requestedKg: 10 }],
    }),
  });
  ok("Create → 201 ORD-*", status === 201 && data?.code?.startsWith("ORD-"));
  ok(
    "In list",
    (await api("/orders?limit=5")).data?.data?.some((o) => o.id === data.id),
  );
  ok(
    "Update → 200",
    (await api(`/orders/${data.id}`, { method: "PUT", body: JSON.stringify({ notes: "test" }) }))
      .status === 200,
  );
  ok(
    "Cancel → 200 cancelled",
    (await api(`/orders/${data.id}/cancel`, { method: "POST" })).data?.status === "cancelled",
  );
  ok(
    "Double cancel → 422",
    (await api(`/orders/${data.id}/cancel`, { method: "POST" })).status === 422,
  );
});
